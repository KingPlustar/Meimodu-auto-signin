...
class MeimoaiAPI:
    ...
    def login(self) -> Response:
        ...
        response = self.session.post(url, json=data.model_dump(), timeout=self.timeout)
...
class MeimoaiCrawler:
    def __init__(self) -> None:
        ...
        self.api = MeimoaiAPI(self.session)
    ...
    def handle_unauthorized_response(self, message: MsgResponse) -> bool:
        if message.code != 401:
            return False
    ...
    def login(self, forced: bool = False) -> bool:
        ...
        match parse_resp(response.json(), LoginData):
            case DataResponse(data=data):
                self.logger.info(f"登录成功，用户昵称：{data.nickname}")
                self.session.headers.update({"Authorization": data.token})
                return True
            case MsgResponse(code=code, message=message):
                self.logger.error(f"登录失败，目标url：{response.url}，响应内容：[{code=}, {message=}]")
                return False
    ...
    def sign_in(self, reauthorized: bool = True) -> bool:
        ...
        match resp := parse_resp(response.json(), bool):
            case DataResponse():
            ...
            case MsgResponse():
                ...
                if reauthorized and delayed_call(
                    lambda: self.handle_unauthorized_response(resp), 
                    delay_before=0.52, 
                    delay_after=0.25
                ):
    ...
    def get_user_info(self, reauthorized: bool = True) -> UserData | None:
        ...
        :param reauthorized: 是否在检测到未授权时尝试登录以获取授权，默认是
        :type reauthorized: bool, optional
        ...
        match resp := parse_resp(response.json(), UserData):
            case MsgResponse():
                self.logger.error(
                    f"获取用户信息失败，目标url：{response.url}，响应内容：\n{resp.model_dump_json(indent=2)}"
                )
