...
def bai_piao(meimo: MeimoaiCrawler) -> bool:
    ...
    if user_info is None:
        ...
        meimo.logger.info(
            f"用户昵称：{user_info.nickname}，当前电量：{user_info.balance}"
        )
    ...
    if user_info is None:
        if user_info_after is not None:
            meimo.logger.info(
                f"签到后电量：{user_info_after.balance}，无法对比签到前电量"
            )
        ...
        meimo.logger.info(
            f"签到前电量：{user_info.balance}，签到后电量：{user_info_after.balance}，增加电量：{delta_balance}"
        )
        if delta_balance <= 0:
...
if __name__ == "__main__":
    ...
    try:
        sys.exit(0 if bai_piao(meimo) else 1)
    except Exception:
        meimo.logger.exception("程序执行异常终止，可能是程序已过时！")
        sys.exit(1)
